...
class pCohesion(object):
    ...
    def updateMeritPenalty(self, queryID, vid, candTag, candDeg):
        ...
        meritVid = candDeg[vid] / math.pow(self.verDeg[vid], 3) * contriCounts * commonNeiWithQuery
        ########## Penalty computation ##########
        ...
        for nid in nSet:
            if idx < threshold:
                ...
                break
        if penaltyCount == 0: penaltyCount = 1
        penaltyVid = (self.verPDeg[vid] - candDeg[vid]) / len(self.neiSet[vid]) / penaltyCount  # update penalty
